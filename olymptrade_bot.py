...
# Exact assets from your Olymp Trade account
ASSETS = {
"Forex OTC": [
...
logger = logging.getLogger(__name__)def generate_signal():
...
arrow = "GREEN" if direction == "BUY" else "RED"
signal = (
"NEW SIGNAL!\n\n"
"Trade: " + asset + "\n"
...
direction = random.choice(["BUY", "SELL"])
expiry = random.choice(EXPIRY_OPTIONS)
confidence = random.randint(70, 92)signals.append({
"asset": asset,
"category": category,
"direction": direction,
"expiry": expiry,
...
arrow = "GREEN" if s["direction"] == "BUY" else "RED"
lines.append(
"Signal " + str(i) + "\n"
"Trade: " + s["asset"] + "\n"
"Timer: " + str(s["expiry"]) + " minutes\n"
"Entry: " + entry_time.strftime("%I:%M %p") + "\n"
"Direction: " + s["direction"] + " [" + arrow + "]\n"
